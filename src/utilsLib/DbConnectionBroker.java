...
/**
 * DbConnectionBroker A servlet-based broker for database connections. Creates and manages a pool of
...
public class DbConnectionBroker implements Runnable {
	class ActiveConnectionData {
		String stack;
		Date gotTime = new Date();
	}

	public static HashMap<Connection, ActiveConnectionData> activeConnections = new HashMap<Connection, ActiveConnectionData>();
	public static boolean ACTIVE_CONNECTIONS_DEBUG = true;

	private int unavaiableErrors;
	...
	private int currConnections, connLast, minConns, maxConns, maxConnMSec, maxCheckoutSeconds, initialGettingSleep, minGettingSleep, gettingSleepDec, maxGettingLoops, debugLevel;
	 ...
	 * maxCheckoutSeconds: Max time a connection can be checked out before being recycled. Zero
	 * value turns option off, default is 60 seconds. debugLevel: Level of debug messages output to
	 * the log file. 0 -> no messages, 1 -> Errors, 2 -> Warnings, 3 -> Information
	 */
	public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, int initialGettingSleep,
			int minGettingSleep, int gettingSleepDec, int maxGettingLoops) throws IOException {

		setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, logFileString, maxConnTime, false, DEFAULTMAXCHECKOUTSECONDS, initialGettingSleep, minGettingSleep, gettingSleepDec,
				maxGettingLoops, DEFAULTDEBUGLEVEL);
	}
	...
	public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, boolean logAppend,
			int initialGettingSleep, int minGettingSleep, int gettingSleepDec, int maxGettingLoops) throws IOException {

		setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, logFileString, maxConnTime, logAppend, DEFAULTMAXCHECKOUTSECONDS, initialGettingSleep, minGettingSleep,
				gettingSleepDec, maxGettingLoops, DEFAULTDEBUGLEVEL);
	}
	 ...
	 */
	public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, boolean logAppend,
			int maxCheckoutSeconds, int initialGettingSleep, int minGettingSleep, int gettingSleepDec, int maxGettingLoops, int debugLevel) throws IOException {

		setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, logFileString, maxConnTime, logAppend, maxCheckoutSeconds, initialGettingSleep, minGettingSleep, gettingSleepDec,
				maxGettingLoops, debugLevel);
	}

	private void setupBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns, int maxConns, String logFileString, double maxConnTime, boolean logAppend,
			int maxCheckoutSeconds, int initialGettingSleep, int minGettingSleep, int gettingSleepDec, int maxGettingLoops, int debugLevel) throws IOException {
		...
		this.initialGettingSleep = initialGettingSleep;
		this.minGettingSleep = minGettingSleep;
		this.gettingSleepDec = gettingSleepDec;
		this.maxGettingLoops = maxGettingLoops;
	 ...
	 * corrupted connections and prevents the buildup of open cursors. (Open cursors result when the
	 * application fails to close a Statement). This method acts as fault tolerance for bad
		...
		while (forever) {
			...
			try {
				...
				if (curr_pid.equals(pid)) {
					...
					// Close all connections silently - they are definitely
					// dead.
			...
			for (int i = 0; i < currConnections; i++) { // Do for each
				...
				try { // Test the connection with createStatement call
						...
						if (connStatus[i] > 0) { // In use, catch it next
							...
							if (maxCheckoutMillis != 0) {
								if (timeInUse > maxCheckoutMillis) {
									if (debugLevel > 1) {
										log.println("Warning. Connection " + i + " failed to be returned in time.  Recycling...");
									}
									/*-ADDON */log.println(new Date().toString() + " Timeout (maxCheckoutMillis): " + i);
					...
					if (age > maxConnMSec) { // Force a reset at the max conn
						// time
						log.println(new Date().toString() + " Timeout (age): " + i);
						throw new SQLException();
					}
	...
	/**
	 * This method hands out the connections in round-robin order. This prevents a faulty connection
	 * from locking up an application entirely. A browser 'refresh' will get the next connection
	 * while the faulty connection is cleaned up by the housekeeping thread.
	 * 
	 * If the min number of threads are ever exhausted, new threads are added up the the max thread
	 * count. Finally, if all threads are in use, this method waits 2 seconds and tries again, up to
	 * ten times. After that, it returns a null.
	 */
	public Connection getConnection() {
		...
		if (available) {
			...
			int currGettingSleep = initialGettingSleep;

			for (int outerloop = 1; outerloop <= maxGettingLoops; outerloop++) {
				...
				if (gotOne) {
					...
					try {
						Thread.sleep(currGettingSleep);

						if (currGettingSleep > minGettingSleep) {
							currGettingSleep -= gettingSleepDec;
						}
					} catch (InterruptedException e) {
					}

					// para poder saber qts erros de pegar sem conexão houve.
					unavaiableErrors++;

					if (debugLevel > 0) {
						if (id == null) {
							id = Utils.stuffStr((Utils.getRandomNumber() % 1000) + "", "0", 4, false);
						}

						log.println("[" + id + "](" + Utils.dateTimeToStr(new Date()) + ") wait " + currGettingSleep + ", loop " + String.valueOf(outerloop));

						// ------------------------------------
						// exibe onde estão as conexões, já que deu pau para
						// conseguir uma nova (se tiver debugando)
						if (ACTIVE_CONNECTIONS_DEBUG && conn == null) {
							try {
								int printRandom = Utils.getRandomNumber() % 1000;

								for (Connection activeConn : activeConnections.keySet().toArray(new Connection[activeConnections.size()])) {
									ActiveConnectionData where = activeConnections.get(activeConn);

									if (where != null) {
										JSPUtils.log("active_connections", "[" + printRandom + "] " + activeConn + " | " + Utils.dateTimeToStr(where.gotTime) + " - " + where.stack);
									}
								}
							} catch (Exception e) {
							}
						...
						// Utils.timeToStr(new Date()) + ") Next will wait " +
						// currGettingSleep + " and try again and loop " +
		...
		if (debugLevel > 2) {
			log.println("Handing out connection " + idOfConnection(conn) + " --> " + (new SimpleDateFormat("MM/dd/yyyy  hh:mm:ss a")).format(new java.util.Date()));
		}

		// ------------------------------------
		// guarda a conexão e onde está
		if (ACTIVE_CONNECTIONS_DEBUG && conn != null) {
			try {
				throw new RuntimeException("");
			} catch (Exception error) {
				ActiveConnectionData activeConnectionData = new ActiveConnectionData();
				activeConnectionData.stack = Utils.getStackTrace(error, false);

				activeConnections.put(conn, activeConnectionData);
			}
		}
		// ------------------------------------
	...
	/**
	 * Shuts down the housekeeping thread and closes all connections in the pool. Call this method
	 * from the destroy() method of the servlet.
	 */
	 ...
	 * <code>freeConnection()</code> can still be called to return used connections.
	 * <LI>After <code>millis</code> milliseconds after the shutdown of the housekeeping thread, all
	 * connections in the pool are closed.
	 * <LI>If any connections were in use while being closed then a <code>SQLException</code> is
	 * thrown.
	...
	/**
	 * Less safe shutdown. Uses default timeout value. This method simply calls the
	 * <code>destroy()</code> method with a <code>millis</code> value of 10000 (10 seconds) and
	 * ignores <code>SQLException</code> thrown by that method.
	 * 
	 * @see #destroy(int)
	 */
	public void destroy() {
	...
	public int getUnavaiableErrors() {
		return unavaiableErrors;
	}

	public void setUnavaiableErrors(int unavaiableErrors) {
		this.unavaiableErrors = unavaiableErrors;
	}
